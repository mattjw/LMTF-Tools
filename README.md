...
This repository contains open-source software tools for converting multilayer network data files into the LASAGNE Multilayer Network Framework format.
...
connector_brain
connector_pardus
generate_multiplex_text
generate_multiplex_db
generate_multiplex_brain_text
generate_multiplex_brain_db
draw_multiplex_db
draw_multiplex_file
```
...
Connectors are provided for three input data formats. In all formats, inputs are raw text values (generally .txt files), separated by commas. The formats are as follows.
...
The format of the data is the Pajek format. More information on Pajek can be found [here](http://pajek.imfm.si/doku.php).
...
Command line arguments for each of these scripts are: 

```mySQL username, mySQL password, database file name, table name```
...
* `connector_flight` converts raw data of the Flight data set into meta format.<br/>
  Command line arguments: name of the network and flight file; e.g., `python connector_flight.py flight flight.txt`
* `connector_terrorist` converts raw data of the Noordin Top terrorist data set into meta format.<br/>
  Command line arguments: Name of the network and terrorist files; e.g., `python connector_terrorist.py terrorist a.txt b.txt ...`
* `connector_pardus` converts raw data of the Pardus data set into meta format.<br/>
  Command line arguments: Name of the network and pardus files; e.g., `python connector_pardus.py pardus a.txt b.txt ...`
* `connector_brain` converts raw data of the Brain data set into meta format.<br/>
  Command line arguments: name of the network and files; e.g., `python connector_brain.py brain a.txt b.txt ...`
...
* Text output:
    ...
    * `generate_multiplex_text` is a script which creates multiplex format for input datasets and stores them in a text file.<br/>
      Command line arguments: `name of the network`

    * `generate_multiplex_brain_text` is a script which creates multiplex format for the brain input dataset and outputs to a text file.<br/>
      Command line arguments: `name of the network`
...
* SQL output:
	...
	* `generate_multiplex_db` is a script which creates multiplex format for input datasets and stores them in a SQL database.<br/>
	  Command line arguments: `MySQL username, MySQL password, name of the network`
	
	* `generate_multiplex_brain_db` is a script which creates multiplex format for the brain input dataset and outputs to a SQL database.<br/>
	 Command line arguments: `MySQL username, MySQL password, name of the network`
