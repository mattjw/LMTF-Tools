""" 
Centrality measures on multiplex network by taking datas from database
"""
...
class Centrality():
    ...
    def dic_of_node(self, cursor, nodelist):
        node_list = {}  # dictionary keeps nodeid as an item and counter as a value
        cursor.execute('''SELECT * FROM ''' + nodelist )
        row = cursor.fetchone()
        counter = 1
        while row is not None:
            node_list[int(row[0])] = counter
            row = cursor.fetchone()
            counter += 1
        return node_list
        
    # put the layers in a dictionary
    def dic_of_layer(self, cursor, layerlist):
        layer_list = {}  # dictionary keeps nodeid as an item and counter as a value
        cursor.execute('''SELECT * FROM ''' + layerlist )
        row = cursor.fetchone()
        counter = 1
        while row is not None:
            layer_list[int(row[0])] = counter
            row = cursor.fetchone()
            counter += 1
        return layer_list
            
    # create adjacency matrix
    def create_matrix(self, cursor, multiplex, node, layer, dic_of_nodes, dic_of_layers):
        ...
        while row is not None:
            ...
            """
            m = int(dic_of_nodes[int(row[0])]) + node * (int(dic_of_layers[int(row[2])])-1) - 1
            # and n= 10+ node*(1-1) -1 , here n=9
            n = int(dic_of_nodes[int(row[1])]) + node * (int(dic_of_layers[int(row[3])])-1) - 1
...
def main(arg1, arg2, arg3):
    ...
    # select the database
    mt.select_db(cursor, db_name)
    # list of layers
    list_of_layers = mt.list_of_layer(cursor, layerlist)
    # length of layer list
    layer = len(list_of_layers)
    # list of nodes
    list_of_nodes = mt.list_of_node(cursor, nodelist)
    # dictionary of nodes
    dic_of_nodes = mt.dic_of_node(cursor, nodelist)
    # dictionary of layers
    dic_of_layers = mt.dic_of_node(cursor, layerlist)
    # length of node list
    node = len(list_of_nodes)
    # create adjacency matrix
    M = mt.create_matrix( cursor, multiplex, node, layer, dic_of_nodes, dic_of_layers)
    # degree centrality of each node in each layer
    ...
    print str("union centrality\n")+str(
    max(uni.iterkeys(), key=lambda k: uni[k])) + ":"  + str(uni[max(uni.iterkeys(), key=lambda k: uni[k])])
    ...
    print str("betweeenness centrality\n")+str(
